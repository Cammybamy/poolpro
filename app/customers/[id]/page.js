...
export default function CustomerDetail({ params }) {
  ...
  async function deletePhoto(url) {
    const newPhotos = (customer.equipment_photos || []).filter(p => p !== url)
    await supabase.from('customers').update({ equipment_photos: newPhotos }).eq('id', id)
      ...
      <div className="p-4 max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-white rounded-xl shadow p-4">
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            {(customer.equipment_photos || []).map((url, i) => (
              <div key={i} className="relative">
                <img src={url} alt="Equipment" className="w-full h-32 object-cover rounded-lg" />
                <button onClick={() => deletePhoto(url)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 text-xs flex items-center justify-center">✕</button>
              </div>
            ))}
          </div>
